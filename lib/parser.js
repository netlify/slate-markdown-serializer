...
Renderer.prototype.groupTextInRanges = function (childNode) {
  ...
  var out = node.reduce(function (acc, current) {
    ...
    if (current instanceof TextNode) {
      ...
      return acc.concat(_this.groupTextInRanges(current));
    } else {
      acc.push(current);
      return acc;
    }
    ...
    case "list_item_start":
      {
        ...
        while (this.next().type !== "list_item_end") {
          _body3.push(this.token.type === "text" ? this.parseText() : this.tok());
        }
    ...
    case "loose_item_start":
      {
        ...
        while (this.next().type !== "list_item_end") {
          _body4.push(this.token.type === "text" ? this.parseText() : this.tok());
        }
